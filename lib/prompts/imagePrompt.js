...
export function buildImagePrompt({ character, emotion, action, props, fullText, seed }) {
  ...
  const colorClause = conventional
    ? ` ${String(conventional).replace(/^Conventional colors:/i, "Colors:")}`
    : "";
  ...
  // NOTE: 템플릿(ComfyUI 워크플로) 내부에 스타일/룩이 있으므로,
  // 여기서는 "내용(무엇/무슨 행동/소품/색)"만 간단하게 전달합니다.
  const prompt = `${subject} ${actionClause}.${propSuffix}${colorClause}`.replace(/\s+/g, " ").trim();
